...
const createPositiveHint = (english: string) => {
    ...
    const cleanEnglish = english.replace(/^to be /, 'being ').replace(/^to /, 'the act of ').toLowerCase();
...
export default function CrosswordGame({ items }: WordleGameProps) {
    ...
    const generateCrossword = useCallback(() => {
        ...
        setTimeout(() => {
            ...
            while (attempts < 10 && !success) {
                ...
                placed.push({
                    id: w1.id, stoneyClean: w1Clean, stoneyDisplay: w1.stoney, english: w1.english,
                    hint: createPositiveHint(w1.english), row: 0, col: 0, isHorizontal: true, number: 1
                });
                ...
                for (let w of wordsList) {
                    ...
                    for (const coord of testCoords) {
                        ...
                        if (splitIdx !== -1) {
                            ...
                            const isHoriz = placed.some(p => p.isHorizontal && p.row === r && c >= p.col && c < p.col + p.stoneyClean.length);
                            ...
                            for (let i = 0; i < cleanWord.length; i++) {
                                ...
                                if (board.has(tCoord) && board.get(tCoord) !== cleanWord[i]) isValid = false;

                                // Adjacency check: Keep parallel words from accidentally gluing together
                                if (!board.has(tCoord)) {
                                    if (tryHorizontal) {
                                        // Check directly above and below the horizontal word
                                        if (board.has(`${tr - 1},${tc}`) || board.has(`${tr + 1},${tc}`)) isValid = false;
                                        // Check one space before the start and after the end
                                        if (i === 0 && board.has(`${tr},${tc - 1}`)) isValid = false;
                                        if (i === cleanWord.length - 1 && board.has(`${tr},${tc + 1}`)) isValid = false;
                                    } else {
                                        // Check directly left and right of the vertical word
                                        if (board.has(`${tr},${tc - 1}`) || board.has(`${tr},${tc + 1}`)) isValid = false;
                                        // Check one space above the start and below the end
                                        if (i === 0 && board.has(`${tr - 1},${tc}`)) isValid = false;
                                        if (i === cleanWord.length - 1 && board.has(`${tr + 1},${tc}`)) isValid = false;
                                    }
                                }
                            }
                            ...
                            if (isValid) {
                                placed.push({
                                    id: w.id, stoneyClean: cleanWord, stoneyDisplay: w.stoney, english: w.english,
                                    hint: createPositiveHint(w.english), row: startR, col: startC, isHorizontal: tryHorizontal, number: nextNumber++
                                });
                                for (let i = 0; i < cleanWord.length; i++) {
                                    const tr = tryHorizontal ? startR : startR + i;
                                    const tc = tryHorizontal ? startC + i : startC;
                    ...
                    placed.forEach(p => {
                        for (let i = 0; i < p.stoneyClean.length; i++) {
                            ...
                            if (!newGridMap.has(coord)) {
                                newGridMap.set(coord, { letter: p.stoneyClean[i], wordIds: [p.id], userLetter: '', isLocked: false, row: r, col: c });
                            } else {
    ...
    const handleKeyPress = useCallback((key: string) => {
        ...
        if (key === '⌫') {
            ...
            if (!cell.isLocked) {
                ...
                if (allLocked && anyChanges) { // won just now
                    wonGame = true;
                }
    ...
    if (gameState === 'loading' || placedWords.length === 0) {
        return (
            <View style={styles.center}>
                ...
                <Text style={{ marginTop: 12, color: colors.textMutedDark }}>Generating puzzle...</Text>
            </View>
        );
    }
    ...
    if (selectedCell) {
        ...
        if (cell) {
            currentWord = placedWords.find(p => p.isHorizontal === (direction === 'horizontal') && cell.wordIds.includes(p.id));
            if (!currentWord) {
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
            ...
            <View style={[styles.hintBox, gameState === 'won' ? styles.hintBoxWon : {}]}>
                {gameState === 'won' ? (
                    ...
                    <>
                        <Text style={styles.clueNumber}>{currentWord.number} {currentWord.isHorizontal ? 'Across' : 'Down'}</Text>
                        <Text style={styles.hintMessage}>{currentWord.hint}</Text>
                        <Text style={styles.hintCategory}>(Needs {currentWord.stoneyClean.length} letters)</Text>
                    </>
                ) : (
                    ...
                    return (
                        <View key={`r${r}`} style={styles.row}>
                            {Array.from({ length: maxC - minC + 1 }).map((_, cIdx) => {
                                ...
                                if (!cell) {
                                    return <View key={`c${c}`} style={[styles.emptyBox, { width: cellSize, height: cellSize }]} />;
                                }

                                const isSelected = selectedCell?.r === r && selectedCell?.c === c;
                                const isHighlight = currentWord && cell.wordIds.includes(currentWord.id);

                                let bg = colors.surface;
                                if (isSelected) bg = '#ffda00'; // NYT yellow
            ...
            {/* All Clues List */}
            {gameState === 'playing' && (
                <View style={[styles.allCluesContainer, isMobile ? { flexDirection: 'column' } : { flexDirection: 'row' }]}>
                    <View style={styles.clueColumn}>
                        <Text style={styles.clueColumnHeader}>Across</Text>
                        {placedWords.filter(p => p.isHorizontal).sort((a, b) => a.number - b.number).map(p => (
                            <Pressable
                                key={p.id}
                                onPress={() => handleCellClick(p.row, p.col)}
                            >
                                <Text style={[styles.clueItem, currentWord?.id === p.id && styles.clueItemActive]}>
                                    <Text style={styles.clueItemNumber}>{p.number}.</Text> {p.hint}
                                </Text>
                            </Pressable>
                        ))}
                    </View>
            ...
            {gameState === 'won' && (
                <View style={styles.answersList}>
                    ...
                    {placedWords.sort((a, b) => a.number - b.number).map(p => (
                        <Text key={p.id} style={styles.answerText}>
                            <Text style={{ fontWeight: 'bold' }}>{p.number} {p.isHorizontal ? 'Across' : 'Down'}:</Text> {p.stoneyDisplay} <Text style={{ color: colors.textMutedDark, fontStyle: 'italic' }}>({p.english})</Text>
                        </Text>
                    ))}
                </View>
            )}
...
const createStyles = (colors: any) => StyleSheet.create({
    ...
    scrollContent: { paddingBottom: 40, alignItems: 'center', width: '100%', backgroundColor: '#fcfbfa' },
    header: {
        flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between',
        width: '100%', paddingVertical: 12, paddingHorizontal: 16, maxWidth: 600
    },
    title: { fontSize: 18, fontWeight: 'bold', color: '#000', fontFamily: 'Georgia, serif', textTransform: 'uppercase', letterSpacing: 1 },
    newGameBtn: {
        backgroundColor: '#000', paddingHorizontal: 12, paddingVertical: 8, borderWidth: 1, borderColor: '#000', borderRadius: 0
    },
    newGameLabel: { color: colors.surface, fontWeight: 'bold', fontSize: 12, fontFamily: 'Courier New, monospace', textTransform: 'uppercase' },
    hintBox: {
        backgroundColor: colors.surface, padding: 16, marginBottom: 24,
        width: '90%', maxWidth: 600, borderWidth: 2, borderColor: '#000', minHeight: 90,
        justifyContent: 'center', shadowColor: '#000', shadowOffset: { width: 4, height: 4 }, shadowOpacity: 1, shadowRadius: 0
    },
    hintBoxWon: { backgroundColor: '#f8f8f8', borderColor: '#000', alignItems: 'center' },
    clueNumber: { fontSize: 14, fontWeight: '700', color: '#000', marginBottom: 6, textTransform: 'uppercase', fontFamily: '"Courier New", Courier, monospace' },
    hintMessage: { fontSize: 18, fontStyle: 'italic', color: '#111', marginBottom: 8, fontFamily: 'Georgia, serif' },
    hintCategory: { fontSize: 13, color: '#444', fontFamily: 'Georgia, serif' },
    gridOuter: {
        marginBottom: 32, padding: 2, backgroundColor: '#000',
    ...
    boxLocked: {},
    boxNumber: {
        position: 'absolute', top: 2, left: 3, fontSize: 11, fontWeight: 'bold', color: '#000', fontFamily: 'Arial, sans-serif'
    },
    ...
    key: {
        backgroundColor: colors.surface, paddingVertical: 14, paddingHorizontal: 12, borderWidth: 1, borderColor: '#000', borderRadius: 0,
        alignItems: 'center', justifyContent: 'center', shadowColor: '#000', shadowOffset: { width: 2, height: 2 }, shadowOpacity: 1, shadowRadius: 0
    },
    keyText: { fontSize: 16, fontWeight: 'bold', color: '#000', fontFamily: 'Arial, sans-serif' },
    allCluesContainer: {
        width: '90%', maxWidth: 800, gap: 32, marginTop: 16, marginBottom: 32, paddingHorizontal: 8,
    },
    clueColumn: { flex: 1 },
    clueColumnHeader: {
        fontSize: 20, fontWeight: 'bold', color: '#000', marginBottom: 16,
        borderBottomWidth: 3, borderBottomColor: '#000', paddingBottom: 6,
        fontFamily: 'Georgia, serif', textTransform: 'uppercase'
    },
    clueItem: { fontSize: 15, color: '#111', marginBottom: 10, lineHeight: 22, padding: 4, fontFamily: 'Georgia, serif' },
    clueItemActive: { fontWeight: 'bold', color: '#000', backgroundColor: '#a7d8ff' },
    clueItemNumber: { fontWeight: 'bold', color: '#000' },
    answersList: { width: '90%', maxWidth: 600, marginTop: 24, padding: 16, backgroundColor: colors.surface, borderWidth: 2, borderColor: '#000' },
    answersHeader: { fontSize: 18, fontWeight: 'bold', marginBottom: 12, color: '#000', fontFamily: 'Georgia, serif' },
